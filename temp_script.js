        ...
        window.viewDetails = (id) => {
            ...
            const sortedHistory = [...t.history].sort((a,b) => a.timestamp - b.timestamp);
            ...
            const htmlItems = sortedHistory.map(h => {
                ...
                return `<div class="p-4 rounded-2xl border ${h.type === 'buy' ? 'bg-indigo-50/50 border-indigo-100 dark:bg-indigo-900/10 dark:border-indigo-900/30' : 'bg-slate-50 border-slate-200 dark:bg-slate-800 dark:border-slate-700'}">
                    ...
                    <div id="note-view-${h.timestamp}" class="group relative">
                        ...
                        <button onclick="toggleEditNote('${t.id}', ${h.timestamp})" class="absolute top-2 right-0 p-1 text-slate-300 hover:text-indigo-500 transition-colors"><i data-lucide="edit-2" class="w-3 h-3"></i></button>
                    </div>

                    <div id="note-edit-${h.timestamp}" class="hidden mt-2 pt-2 border-t border-dashed border-slate-200 dark:border-slate-700">
                        <textarea id="note-input-${h.timestamp}" rows="2" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-sm font-bold focus:ring-2 focus:ring-indigo-500 mb-2">${h.note || ''}</textarea>
                        <div class="flex justify-end gap-2">
                            <button onclick="toggleEditNote('${t.id}', ${h.timestamp})" class="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">取消</button>
                            <button onclick="saveHistoryNote('${t.id}', ${h.timestamp})" class="px-3 py-1.5 rounded-lg text-xs font-bold bg-indigo-600 text-white hover:bg-indigo-700">儲存</button>
                        </div>
                    </div>
        ...
        window.toggleEditNote = (tradeId, timestamp) => {
            const viewEl = document.getElementById(`note-view-${timestamp}`);
            const editEl = document.getElementById(`note-edit-${timestamp}`);
            if(viewEl && editEl) {
        ...
        window.saveHistoryNote = async (tradeId, timestamp) => {
            const t = trades.find(item => item.id === tradeId);
            if(!t) return;

            const newContent = document.getElementById(`note-input-${timestamp}`).value;
            const newHistory = t.history.map(h => {
                if(h.timestamp === timestamp) {
                    return { ...h, note: newContent };
                }
                return h;
            });
            ...
            try {
                ...
                // Note: The UI will update automatically via onSnapshot, but we might want to manually toggle back if snapshot is slow.
                // However, since onSnapshot calls render, viewDetails might be closed or refreshed.
                // Actually viewDetails is NOT re-rendered by onSnapshot unless we call viewDetails again.
                // But onSnapshot calls renderPortfolio/History.
                // Let's manually update the UI in the modal to reflect change immediately without closing it.

                // We need to update the global trades array temporarily? No, onSnapshot will handle it.
                // But viewDetails shows static HTML generated at click time.
                // So we should re-render viewDetails with the new data?
                ...
                document.getElementById(`note-input-${timestamp}`).value = newContent;
